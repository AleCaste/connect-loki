...
module.exports = (session) => {
  ...
  function LokiStore (options) {
    ...
    // [Alex]
    this.ttl = ((options.ttl>=0)?options.ttl : 1209600)
    if (this.ttl === 0) { this.ttl = null }
    this.ttlInterval = ((options.ttlInterval>=0)?options.ttlInterval : 5000);
    ...
    this.client = new Loki(this.storePath, {
      ...
      autosaveInterval: self.ttlInterval
    })
    ...
    this.client.loadDatabase({}, () => {
      ...
      if (_.isNil(self.collection)) {
        self.collection = self.client.addCollection('Sessions', {
          ...
          // [Alex]
          ttl: self.ttl,                  // age
          ttlInterval: self.ttlInterval   // interval (equal to autosaveInterval)
        })
      }
